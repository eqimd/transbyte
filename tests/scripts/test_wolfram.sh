...
mkdir debug &&
cd ../../ &&
./gradlew run --args="-c tests/examples/WolframGenerator/WolframGenerator.class -s WolframGenerator -m generate:([Z)[Z --output tests/scripts/debug/wolfram_java.aag --array-sizes 128" &&
cd tests/scripts/ &&
